...
void N(float T, float &ε, float &Δψ, float &Δε){
    float T2=T*T;
    float T3=T2*T;
    float ARG;
...
void P(float T, float &z, float &ϑ, float &ζ){
    float T2=T*T;
    float T3=T2*T;
...
QMatrix3x3 R(float JDt, float T){ // T=TT(t1); where t1=QDateTime::currentDateTimeUtc() and t=t1.toSecsSinceEpoch();
